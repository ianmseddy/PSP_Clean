...
#    `functionName` is a function's name in your module (e.g., `PSP_CleanEvent1`).
# 2. Copy this file to the tests folder (i.e., `C:/Ian/Git/PSP_Clean/modules/PSP_Clean/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("PSP_Clean")
  path <- list(modulePath = "C:/Ian/Git/PSP_Clean/modules",
               outputPath = file.path(tempdir(), "outputs"))
  parameters <- list(
    #.progress = list(type = "graphical", interval = 1),
    .globals = list(verbose = FALSE),
    PSP_Clean = list(.saveInitialTime = NA)
  )
  ...
  if (exists("PSP_CleanEvent1", envir = .GlobalEnv)) {
    simOutput <- PSP_CleanEvent1(mySim)
  } else {
  ...
  if (exists("PSP_CleanEvent2", envir = .GlobalEnv)) {
    simOutput <- PSP_CleanEvent2(mySim)
  } else {
